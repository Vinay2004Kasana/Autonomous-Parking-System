...
class ParkingSystem:
    ...
    def run(self, display=True, gate_sim_every=90):
        ...
        # ASCII-only window name avoids Unicode crash on Windows (OneDrive paths)
        WIN_NAME = "ParkVision"
        if display:
...
def find_video_source():
    """
    Auto-searches for a video file in Car-Videos/ folder.
    Falls back to any .mp4/.avi in the current directory.
    ...
    extensions = ["*.mp4", "*.avi", "*.mov", "*.mkv", "*.MOV", "*.MP4"]

    for folder in search_dirs:
        for ext in extensions:
            pattern = str(Path(folder) / ext)
            matches = glob.glob(pattern)
            if matches:
                return matches[0]   # return first found video
    return None
...
if __name__ == "__main__":
    ...
    ap.add_argument("--source",  default=None,
                    help="Path to video file. If not given, auto-searches Car-Videos/")
    ap.add_argument("--headless", action="store_true")
    args = ap.parse_args()

    # Resolve source
    if args.source is None:
        src = find_video_source()
        if src is None:
            print("[ERROR] No video file found in Car-Videos/ folder.")
            print("        Put a .mp4 file there or pass --source path/to/video.mp4")
            exit(1)
        print(f"[AUTO] Using video: {src}")
    ...
    ParkingSystem(weights="best.pt", video_source=src).run(display=not args.headless)
